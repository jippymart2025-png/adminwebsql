...
`);
    ...
    $(document).on("click","input[name='isActive']",async function(e) {
        ...
        var planName = '';
        try {
            var doc = await database.collection('subscription_plans').doc(id).get();
            if (doc.exists) {
        ...
        if(ischeck) {
            database.collection('subscription_plans').doc(id).update({
                ...
                await logActivity('subscription_plans', 'enabled', 'Enabled subscription plan: ' + planName);
            });
        } else {
            ...
            if(EnabledSubscriptions==1) {
                ...
                database.collection('subscription_plans').doc(id).update({
                    ...
                    await logActivity('subscription_plans', 'disabled', 'Disabled subscription plan: ' + planName);
                });
            }
    ...
    $(document).on("click","a[name='plan-delete']",async function(e) {
        ...
        // Get plan name before deletion for logging
        var planName = '';
        try {
            var doc = await database.collection('subscription_plans').doc(id).get();
            if (doc.exists) {
        ...
        database.collection('subscription_plans').doc(id).delete().then(async function(result) {
            await logActivity('subscription_plans', 'deleted', 'Deleted subscription plan: ' + planName);
        ...
        if($('#subscriptionPlansTable .is_open:checked').length) {
            if(confirm("{{ trans('lang.selected_delete_alert') }}")) {
                ...
                $('#subscriptionPlansTable .is_open:checked').each(async function() {
                    ...
                    var planName = '';
                    try {
                        var doc = await database.collection('subscription_plans').doc(dataId).get();
                        if (doc.exists) {
                            planName = doc.data().name;
                        }
                    } catch (error) {
