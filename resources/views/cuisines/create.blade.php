        ...
        <div class="row page-titles">
            ...
            <div class="col-md-7 align-self-center">
                <ol class="breadcrumb">
                    ...
                    <li class="breadcrumb-item"><a href="{!! route('cuisines') !!}">{{trans('lang.cuisines_plural')}}</a>
                    </li>
                    <li class="breadcrumb-item active">{{trans('lang.cuisines_create')}}</li>
                </ol>
            </div>
        </div>
            ...
            <div class="cat-edite-page max-width-box">
                <div class="card  pb-4">
                        ...
                        <div class="row restaurant_payout_create" role="tabpanel">
                            ...
                            <textarea rows="7" class="category_description form-control"
                                      id="category_description"></textarea>
                                                <div class="form-text text-muted">{{ trans("lang.cuisines_description_help")
                                                }}
                                                </div>
                                            </div>
                                        </div>
                                        <div class="form-group row width-100">
                                            ...
                                            <div class="col-7">
                                                ...
                                                <div class="form-text text-muted w-50">{{ trans("lang.cuisines_image_help")
                                                }}
                                                </div>
                                            </div>
                                        </div>
                    ...
                    <div class="form-group col-12 text-center btm-btn">
                        ...
                        <a href="{!! route('cuisines') !!}" class="btn btn-default"><i class="fa fa-undo"></i>{{trans('lang.cancel')}}</a>
                    </div>
                </div>
            </div>
        </div>
...
<script>
    ...
    var ref = database.collection('vendor_cuisines');
    var photo = "";
    var fileName='';
        ...
        $(".save-setting-btn").click(async function () {
            ...
            if (title == '') {
                ...
                var count_vendor_categories = 0;
                    if (show_in_homepage) {
                        await database.collection('vendor_cuisines').where('show_in_homepage', "==", true).get().then(async function (snapshots) {
              ...
              storeImageData().then(IMG => {
                database.collection('vendor_cuisines').doc(id_category).set({
                    'id': id_category,
                    'title': title,
                    ...
                    console.log('✅ Cuisine saved successfully, now logging activity...');
                    
                    // Log the activity with error handling and await the Promise
                    try {
                        if (typeof logActivity === 'function') {
                            console.log('🔍 Calling logActivity for cuisine creation...');
                            await logActivity('cuisines', 'created', 'Created new cuisine: ' + title);
