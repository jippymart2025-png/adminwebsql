...
@section('scripts')
    ...
    <script>
        ...
        // Fetch placeholder image from MySQL
        $.ajax({
            url: apiBaseUrl + '/vendors/placeholder-image',
            method: 'GET',
            success: function(response) {
                if (response.success && response.image) {
                    placeholderImage = response.image;
                } else {
                    placeholderImage = apiBaseUrl + '/assets/images/placeholder-image.png';
                }
            },
            error: function() {
                placeholderImage = apiBaseUrl + '/assets/images/placeholder-image.png';
        ...
        // Fetch zones from MySQL
        $.ajax({
            url: apiBaseUrl + '/vendors/zones',
            method: 'GET',
            success: function(response) {
        ...
        // Fetch delivery charge settings from MySQL
        $.ajax({
            url: apiBaseUrl + '/settings/api/delivery-charge/settings',
            method: 'GET',
            success: function(response) {
                if (response.success && response.data) {
                    var deliveryChargeSettings = response.data;
                    try {
                        if (deliveryChargeSettings.vendor_can_modify) {
                            // Will be set from restaurant data
                        } else {
                            $("#delivery_charges_per_km").val(deliveryChargeSettings.delivery_charges_per_km || 0);
                            $("#minimum_delivery_charges").val(deliveryChargeSettings.minimum_delivery_charges || 0);
                            $("#minimum_delivery_charges_within_km").val(deliveryChargeSettings.minimum_delivery_charges_within_km || 0);
                        ...
                        console.error('Error processing delivery charge settings:', error);
                    }
                }
            }
        });

        // Fetch story settings from MySQL
        $.ajax({
            url: apiBaseUrl + '/settings/api/story/settings',
            method: 'GET',
            success: function(response) {
                if (response.success && response.data) {
                    var story_data = response.data;
        ...
        // Fetch special discount offer settings from MySQL
        $.ajax({
            url: apiBaseUrl + '/settings/api/specialoffer/settings',
            method: 'GET',
            success: function(response) {
                if (response.success && response.data) {
                    var specialDiscountOffer = response.data;
                    specialDiscountOfferisEnable = specialDiscountOffer.isEnable || false;
                }
            }
        });

        // Fetch currency from MySQL
        $.ajax({
            url: apiBaseUrl + '/payments/currency',
            method: 'GET',
            success: function(response) {
                if (response.success && response.currency) {
                    currentCurrency = response.currency.symbol || '';
                    currencyAtRight = response.currency.symbolAtRight || false;
                }
            }
        });
        $(document).ready(function() {
            ...
            $(document).on("click", ".remove-btn", function() {
                ...
                if (status == "old") {
                    galleryImageToDelete.push(photo_remove);
                }
                $("#photo_" + id).remove();
            ...
            $(document).on("click", ".remove-menu-btn", function() {
                ...
                var status = $(this).attr('data-status');
                if (status == "old") {
                    menuImageToDelete.push(photo_remove);
                }
            ...
            $.ajax({
                url: apiBaseUrl + '/restaurants/' + id + '/data',
                method: 'GET',
                headers: {
                ...
                success: function(response) {
                    ...
                    // Fetch vendor cuisines from MySQL
                    $.ajax({
                        url: apiBaseUrl + '/restaurants/cuisines',
                        method: 'GET',
                        success: function(response) {
                            if (response.success && response.data) {
                                response.data.forEach(function(cuisine) {
                                    if (restaurant.vendorCuisineID && cuisine.id == restaurant.vendorCuisineID) {
                    ...
                    // Fetch vendor categories from MySQL
                    $.ajax({
                        url: apiBaseUrl + '/api/vendor-categories',
                        method: 'GET',
                        success: function(response) {
                            if (response.success && response.data) {
                    ...
                    // Set delivery charges from restaurant data if available
                    if (restaurant.DeliveryCharge) {
                        var deliveryCharge = restaurant.DeliveryCharge;
                        if (typeof deliveryCharge === 'string') {
                            deliveryCharge = JSON.parse(deliveryCharge);
                        }
                        $("#delivery_charges_per_km").val(deliveryCharge.delivery_charges_per_km || 0);
                        $("#minimum_delivery_charges").val(deliveryCharge.minimum_delivery_charges || 0);
                        $("#minimum_delivery_charges_within_km").val(deliveryCharge.minimum_delivery_charges_within_km || 0);
                    }

                    // Get story data from restaurant response
                    if (restaurant.story) {
                        var videoUrlData = restaurant.story.videoUrl || [];

                        // Handle videoUrl - it might be a JSON string or array
                        if (typeof videoUrlData === 'string') {
                            try {
                                videoUrlData = JSON.parse(videoUrlData);
                            } catch (e) {
                                // If parsing fails, treat as single URL string
                                videoUrlData = videoUrlData ? [videoUrlData] : [];
                            }
                        }

                        if (!Array.isArray(videoUrlData)) {
                            videoUrlData = videoUrlData ? [videoUrlData] : [];
                        }

                        story_vedios = videoUrlData;
                        story_thumbnail = restaurant.story.videoThumbnail || '';
                        story_thumbnail_oldfile = restaurant.story.videoThumbnail || '';
                        ...
                        console.log('No story data found in restaurant response');
                    }

                    // Display story videos
                    if (story_vedios && story_vedios.length > 0) {
                        var html = '';
                        for (var i = 0; i < story_vedios.length; i++) {
                            if (story_vedios[i]) { // Make sure URL exists
                    ...
                    if (story_thumbnail) {
                        var thumbnailHtml = '<div class="col-md-3"><div class="thumbnail-inner"><span class="remove-story-thumbnail" data-img="' + story_thumbnail + '"><i class="fa fa-remove"></i></span><img id="story_thumbnail_image" src="' + story_thumbnail + '" width="150px" height="150px;"></div></div>';
                        jQuery("#story_thumbnail").html(thumbnailHtml);
                    }
                } catch (error) {
                }
            ...
            $(".edit-form-btn").click(async function() {
                ...
                if (restaurantname == '') {
                    ...
                    storeStoryData().then(function(resStoryVid) {
                        storeImageData().then(function(IMG) {
                            storeGalleryImageData().then(function(GalleryIMG) {
                                storeMenuImageData().then(function(MenuIMG) {
                                    // Prepare story data - merge existing videos with new ones
                                    var allStoryVideos = [];
                                    // Add existing videos (those not removed)
                                    if (story_vedios && story_vedios.length > 0) {
                                        allStoryVideos = allStoryVideos.concat(story_vedios);
                                    }
                                    // Add newly uploaded videos
                                    if (resStoryVid && resStoryVid.length > 0) {
                                        allStoryVideos = allStoryVideos.concat(resStoryVid);
                                    }

                                    var storyData = null;
                                    var finalThumbnail = IMG.storyThumbnailImage || story_thumbnail;

                                    // Only include story data if there are videos or thumbnail
                                    if (allStoryVideos.length > 0 || finalThumbnail) {
                                        if (allStoryVideos.length > 0 && !finalThumbnail) {
                                            jQuery("#data-table_processing").hide();
                                            $(".error_top").show();
                                            $(".error_top").html("");
                                            ...
                                            window.scrollTo(0, 0);
                                            return;
                                        } else if (finalThumbnail && allStoryVideos.length == 0) {
                                            jQuery("#data-table_processing").hide();
                                            $(".error_top").show();
                                            $(".error_top").html("");
                                            $(".error_top").append("<p>{{trans('lang.story_error')}}</p>");
                                    ...
                                    if (storyData) {
                                        updateData.story = storyData;
                                    }

                                    // Update restaurant via MySQL API
                                    $.ajax({
                                        url: apiBaseUrl + '/restaurants/' + id,
                                        method: 'PUT',
                                        headers: {
                                            'X-CSRF-TOKEN': csrfToken,
                                        ...
                                        success: function(response) {
                                            ...
                                            try {
                                                if (typeof logActivity === 'function') {
                                                    ...
                                                    logActivity('restaurants', 'updated', 'Updated restaurant: ' + restaurantname);
        ...
        $(document).on("click", ".remove-story-video", function() {
            ...
            // If it's an existing video URL (not a new upload), we can optionally delete it from server
            if (photo_remove && /^https?:\/\/[^ "]+$/.test(photo_remove) && !/^data:/.test(photo_remove)) {
                // Optionally delete from server - for now just log
                console.log("Video to be removed:", photo_remove);
            }

            $("#story_div_" + id).remove();
            var index = new_added_restaurant_story_filename.indexOf(photo_remove);
            if (index > -1) {
        ...
        $(document).on("click", ".remove-story-thumbnail", function() {
            ...
            // If it's an existing image URL (not a new upload), we can optionally delete it from server
            if (photo_remove && /^https?:\/\/[^ "]+$/.test(photo_remove) && !/^data:/.test(photo_remove)) {
                // Optionally delete from server - for now just log
                console.log("Thumbnail to be removed:", photo_remove);
            }
        ...
        function deleteStoryfromCollection() {
            if (story_vedios.length == 0 && story_thumbnail == '') {
                // Story will be deleted when restaurant is updated with empty story data
                console.log("Story will be removed on save");
            }
        }
                ...
                if (story_thumbnail != '' && story_thumbnail_filename != '') {
                    $.ajax({
                        ...
                        data: {
                            ...
                            folder: 'story/images',
                            filename: story_thumbnail_filename
                        },
                        success: function(response) {
        ...
        function storeGalleryImageData() {
            return new Promise(function(resolve, reject) {
                var newPhoto = [];
                ...
                if (new_added_restaurant_photos.length > 0) {
                    var photoPromises = new_added_restaurant_photos.map(function(resPhoto, index) {
                        return new Promise(function(resolvePhoto, rejectPhoto) {
                            $.ajax({
                                url: apiBaseUrl + '/api/upload-image',
                                method: 'POST',
                                headers: {
                                ...
                                data: {
                                    ...
                                    filename: new_added_restaurant_photos_filename[index]
                                },
                                success: function(response) {
                                    if (response.success && response.url) {
                                        resolvePhoto({ index: index, downloadURL: response.url });
                                    } else {
                                        resolvePhoto({ index: index, downloadURL: resPhoto });
                                    }
                                },
                                error: function(error) {
                                    console.log("ERR uploading gallery image ===", error);
                                    resolvePhoto({ index: index, downloadURL: resPhoto });
                                }
                            });
                        });
                    ...
                    Promise.all(photoPromises).then(function(photoResults) {
                        photoResults.sort(function(a, b) { return a.index - b.index; });
                        var uploadedPhoto = photoResults.map(function(photo) { return photo.downloadURL; });
                        newPhoto = newPhoto.concat(uploadedPhoto);
                        resolve(newPhoto);
                    });
                } else {
                    resolve(newPhoto);
        ...
        function storeStoryData() {
            return new Promise(function(resolve, reject) {
                ...
                if (new_added_restaurant_story.length > 0) {
                    var storyPromises = new_added_restaurant_story.map(function(resStory, index) {
                        return new Promise(function(resolveStory, rejectStory) {
                            // Convert File to FormData for video upload
                            var formData = new FormData();
                            formData.append('file', resStory);
                            formData.append('folder', 'story/videos');
                            formData.append('filename', new_added_restaurant_story_filename[index]);
                            ...
                            $.ajax({
                                ...
                                success: function(response) {
                                    if (response.success && response.url) {
                                        ...
                                        newStory.push(response.url);
                                        $("#video_file").val('');
                                        resolveStory();
                                    } else {
                                        // Fallback: keep the file for now, will be handled on save
                                        console.warn('Video upload endpoint may need to be created');
                                        resolveStory();
                                    }
                                },
                                error: function(error) {
                                    console.error('Upload failed:', error);
                                    // Don't reject, just continue - video will be handled differently
                                    resolveStory();
                                }
                            });
                        });
                    });
                    Promise.all(storyPromises).then(function() {
                        resolve(newStory);
                    }).catch(function(error) {
                        console.error('Error uploading story videos:', error);
                        resolve(newStory);
                    });
                } else {
                    resolve(newStory);
        ...
        function storeMenuImageData() {
            return new Promise(function(resolve, reject) {
                ...
                if (new_added_restaurant_menu.length > 0) {
                    var menuPromises = new_added_restaurant_menu.map(function(menuPhoto, index) {
                        return new Promise(function(resolveMenu, rejectMenu) {
                            $.ajax({
                                url: apiBaseUrl + '/api/upload-image',
                                method: 'POST',
                                headers: {
                                ...
                                data: {
                                    ...
                                    filename: new_added_restaurant_menu_filename[index]
                                },
                                success: function(response) {
                                    if (response.success && response.url) {
                                ...
                                error: function(error) {
                                    console.log("ERR uploading menu image ===", error);
                                    resolveMenu();
                                }
                            });
                        });
        ...
        function deleteOffer(day, count, i) {
            ...
            specialDiscount.push(Saturday);
            specialDiscount.push(sunday);
            // Update will be handled when saving the restaurant
            console.log('Special discount updated locally, will be saved on form submit');
        }
        ...
        function deleteWorkingHour(day, count, i) {
            ...
            workingHours.push(sunday);
            // Update will be handled when saving the restaurant
            console.log('Working hours updated locally, will be saved on form submit');
        }
